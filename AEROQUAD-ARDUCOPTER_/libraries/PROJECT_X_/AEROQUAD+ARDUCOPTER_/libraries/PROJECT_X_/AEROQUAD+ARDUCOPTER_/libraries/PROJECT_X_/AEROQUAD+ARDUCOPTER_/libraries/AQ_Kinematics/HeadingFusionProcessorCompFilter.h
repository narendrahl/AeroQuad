...
float headingCompFilter1 = 1.0 / (1.0 + 0.05);
float headingCompFilter2 = 1 - headingCompFilter1;
int headingGyroZero = 0;
float localHeading = 0.0;
float adjustedGyroHeading =0;
float a=0.16, filtered_heading=0.0, filtered_heading_prev=0.0;
...
void calculateHeading() {
  ...
  adjustedGyroHeading = fmod(gyroHeading,(PI*2));
  ...
  if (adjustedGyroHeading > 0) {
   ...
   filtered_heading=a*getAbsoluteHeading()+(1-a)*filtered_heading_prev;
   filtered_heading_prev = filtered_heading;
   
   
  if (headingGyroZero != gyroZero[ZAXIS] || abs(adjustedGyroHeading-filtered_heading)>100) {
    gyroHeading = getAbsoluteHeading();
    headingGyroZero = gyroZero[ZAXIS];
  }
  // Complementry filter from http://chiefdelphi.com/media/papers/2010
  localHeading = (headingCompFilter1 * adjustedGyroHeading) + (headingCompFilter2 * filtered_heading);

  trueNorthHeading =localHeading;//getAbsoluteHeading();//localHeading;//getAbsoluteHeading();    26/03   //*(cos( kinematicsAngle[XAXIS])+cos( kinematicsAngle[YAXIS]));//
  #if defined UseGPS
    if( compassDeclination != 0.0 ) {

	  trueNorthHeading = trueNorthHeading;// + compassDeclination;				// GPS causing variation in the Declination value
